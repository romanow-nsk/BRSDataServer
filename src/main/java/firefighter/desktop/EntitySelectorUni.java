...
public class EntitySelectorUni<T extends Entity> extends javax.swing.JFrame {
    ...
    public EntitySelectorUni(int x0, int y0,String name, int level,Client main, I_OK ok0) {
        initComponents();
        setTitle(Values.EntityFactory.getEntityNameBySimpleClass(name));
        ok = ok0;
        token = main.debugToken;
        setBounds(x0,y0,470,150);
        setVisible(true);
        try {
            Response<ArrayList<DBRequest>> res = main.service.getEntityList(main.debugToken,name,Values.GetAllModeActual,level).execute();
            if (!res.isSuccessful()) {
